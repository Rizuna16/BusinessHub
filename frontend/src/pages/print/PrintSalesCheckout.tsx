...
export const PrintSalesCheckout: React.FC = () => {
  ...
  useEffect(() => {
    if (businessId && id) {
      authFetch(`${API_BASE}/businesses/${businessId}/sales/${id}`)
  ...
  if (!data) return <ErrorState message="Sales checkout record not found." />;

  return (
    <PrintLayout title="Sales Checkout Record" subtitle={`Record #${data.sales_number}`}>
      <div className="mb-4 text-sm">
        <div className="flex justify-between">
          <span><strong>Date:</strong> {new Date(data.sales_date).toLocaleString()}</span>
          <span><strong>Status:</strong> {data.status}</span>
        </div>
        <div className="mt-1"><strong>Customer ID:</strong> {data.customer_id || 'Walk-in'}</div>
      </div>
