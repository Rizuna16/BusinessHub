...
export const PrintPurchase: React.FC = () => {
  ...
  useEffect(() => {
    if (businessId && id) {
      authFetch(`${API_BASE}/businesses/${businessId}/purchases/${id}`)
  ...
  if (!data) return <ErrorState message="Purchase record not found." />;

  return (
    <PrintLayout title="Purchase Record" subtitle={`Purchase #${data.purchase_number}`}>
      <div className="mb-4 text-sm">
        <div className="flex justify-between">
          <span><strong>Date:</strong> {new Date(data.purchase_date).toLocaleString()}</span>
