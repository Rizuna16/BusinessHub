...
export const Receivings: React.FC = () => {
  const { businessId } = useParams<{ businessId: string }>();
  const navigate = useNavigate();
  const { user } = useAuth();

  const [receivings, setReceivings] = useState<ReceivingResponse[]>([]);
  ...
  const [createForm, setCreateForm] = useState<ReceivingCreateInput>({
    purchase_id: '',
  ...
  const fetchReceivings = useCallback(async () => {
    ...
    try {
      setIsLoading(true);
      setServerError('');
      const res = await apiClient.listReceivings(businessId, {
  ...
  const handleCreateReceiving = async (e: React.FormEvent) => {
    ...
    try {
      const payload: ReceivingCreateInput = {
        purchase_id: createForm.purchase_id,
        inventory_location_id: createForm.inventory_location_id,
        notes: createForm.notes?.trim() || undefined,
      };
      const created = await apiClient.createReceiving(businessId, payload);
      setIsCreateModalOpen(false);
      setSuccessMsg(`Penerimaan ${created.receiving_number} berhasil dibuat.`);
      setTimeout(() => setSuccessMsg(''), 4000);
      navigate(`/businesses/${businessId}/receivings/${created.id}`);
    } catch (err: any) {
      setFormError(err?.message || 'Gagal membuat penerimaan barang.');
    } finally {
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <header className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-2xl font-bold text-slate-900 dark:text-slate-100">Penerimaan Barang</h1>
        ...
        {receivings.length === 0 && !isLoading ? (
          <EmptyState
            ...
            description="Buat penerimaan pertama dari Pembelian yang sudah difinalisasi."
            action={canManage ? <Button onClick={() => setIsCreateModalOpen(true)}>+ Penerimaan Baru</Button> : undefined}
          />
        ) : (
          <>
            {/* Desktop table */}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-200 dark:border-slate-700 text-left text-slate-500 dark:text-slate-400">
                    ...
                    <th className="pb-3 pr-4 font-medium">Lokasi Tujuan</th>
                ...
                <tbody>
                  {receivings.map(r => (
                    <tr
                      ...
                      onClick={() => navigate(`/businesses/${businessId}/receivings/${r.id}`)}
                    >
                      <td className="py-3 pr-4 font-medium text-indigo-600 dark:text-indigo-400">{r.receiving_number}</td>
                ...
                >
                  <Card className="hover:ring-2 hover:ring-indigo-500">
                    <div className="flex items-start justify-between">
                      <div>
                        <p className="font-medium text-indigo-600 dark:text-indigo-400">{r.receiving_number}</p>
        ...
        {isCreateModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 dark:bg-black/70 p-4">
            <div className="w-full max-w-lg rounded-xl bg-white dark:bg-slate-900 shadow-xl p-6 max-h-[90vh] overflow-y-auto">
              <h2 className="text-lg font-bold text-slate-900 dark:text-slate-100 mb-4">Penerimaan Barang Baru</h2>
              ...
              <form onSubmit={handleCreateReceiving} className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Lokasi Penerimaan *</label>
                  <select
                    required
